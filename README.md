...
|---|---|---|
| `state=ok` / `nt` | `sent` | готово; коди зауважень пишуться в `records.warn_codes`; `IGNORED` (бюро відкинуло компонент) — окремий рядок алерту, інші non-benign коди — «з зауваженнями». 4014/5001 приходять майже на кожен запис і в алерт не йдуть |
...
`/health` каже `degraded`, якщо: останній успішний прохід старший за 25 год (дні без файлів — теж успіх; aborted не рахується), або частка `rejected` серед записів, прийнятих за останні `HEALTH_REJECTED_WINDOW_DAYS` днів (14), більша за `HEALTH_REJECTED_MAX_RATIO` (0.02), або `failed` поза лімітом ретраїв, або недоступна папка. Звичайний рівень відмов бюро — ~0.3% на файл; сама наявність `rejected` більше не робить сервіс `degraded` (стара історія відмов лишається в БД назавжди). Поточна частка — у полі `recent_rejections`.
