...
| **`bdate` ≠ дата, закодована в `inn`** | одне з двох хибне; бюро ловить це як CRITICAL 2098 |
| немає номера паспорта в жодному джерелі | нічого відправляти (окремо від «формат») |
| непідтримуваний формат паспорта | не з чого зібрати `docs` |
| немає валідного телефону | `contacts` — обовʼязковий блок |

**Неповний документ рядок НЕ блокує** (з 2026-09-03). Якщо ні знімок заявки, ні `users`,
ні інші заявки клієнта не дають видавця (`dwho`) або дати видачі (`dwdt`), документ
надсилається з тим, що є — відсутні ключі просто опускаються. Бюро відкидає такий док
(IGNORED 3003), але **записує дані угоди**: `2077` спрацьовує лише тоді, коли бюро не має
жодного документа цього субʼєкта, а замір на проді (300 рядків, 2026-09-03) не дав ні
одного 2077. Такі рядки рахуються в `summary.lines_doc_incomplete` і йдуть в алерт — це
єдина видимість дірки в кабінеті після відмови від карантину. Джерело з повним набором
(видавець + дата) завжди перемагає неповне, навіть якщо неповне лежить у знімку.

`csex` (дов.1: 1=Чоловік, 2=Жінка) виводиться з 9-ї цифри `inn` (непарна = Ч):
без нього бюро вішає NOTICE 4014 на **кожен** запис, чим засмічує сигнал зауважень.
Правило перевірене на проді (2026-08-14): 99.84% збігу з незалежним джерелом —
закінченнями по батькові. `bdate` теж закодована в `inn` (перші 5 цифр = дні від
1899-12-31; збіг 99.95%), тому розбіжність = карантин.

Інші опційні словникові поля (`family`/`ceduc`/…) свідомо не надсилаються
...
flock (run.lock)
  ...
  → send_records           (кожен рядок = 1 HTTP-запит; паралельно, пул+rate-cap; повертає touched: set[file_id])
...
**`send_records` (серце проходу) — паралельне з чітким розподілом потоків:**

1. `sendable_records`: `pending` **АБО** (`failed` І `attempts < retry_cap`),
   у порядку `file_id, line_no`.
2. **Auth раз** на головному потоці (`client.ensure_session`) перед fan-out;
   провал auth → одразу `aborted` (весь прохід заблоковано). Воркери **ніколи**
   не авторизуються.
3. **Пул воркерів** `ThreadPoolExecutor(max_workers=ubki_concurrency)` з
   **обмеженим вікном подачі** (в польоті максимум `ubki_concurrency` запитів),
   темп обмежено глобальним `RateLimiter(ubki_max_rps)` (рівне рознесення).
   **Воркер робить лише**: `build_envelope` (ліниво, тому в пам'яті одночасно
   ≤`ubki_concurrency` конвертів) + перевірка 2 МБ (завеликий → `REJECTED`
   локально, без мережі й без rate-слоту) + `client.send_prepared(envelope,
   sessid)` (stateless POST). **Головний потік робить усе решта**: коміт у БД
   (одне SQLite-з'єднання, `check_same_thread=True`), summary, abort.
4. `count_attempt = not is_network_error` — **мережеві збої не палять `attempts`**.
   `is_local_reject` (завеликий конверт) нейтральний для лічильника abort.
5. **Stale-сесія** (`session_expired`: 2014 / 401/403) → головний потік
   ре-авторизується **один раз** (`client.reauth`) і повторно шле лише ці записи.
   Практично холодний шлях (сесія жива до 23:59, прохід о 06:00).
6. Збирає `touched: set[file_id]` — які файли перерахувати після проходу.
7. **Abort:** `network_abort_threshold` (3) послідовних network-like помилок **у
   порядку завершення** → `summary.aborted = True`; головний потік припиняє
   подачу (уже в польоті — доганяються й комітяться), тож понад поріг вилетить
   щонайбільше ~`ubki_concurrency` запитів, а не весь беклог. Решта лишається
   `pending`.
...
| `ok` / `nt` | `er = 0` | `SENT`; коди зауважень → `records.warn_codes`, будь-який `IGNORED` → окремий рядок алерту (втрата даних), решта non-benign → `has_warnings` |
...
|----------|-------------|
| `GET /health` | `degraded`, коли остання **успішна** run старша за 25h (дні без файлів — здорові; `aborted` не рахується), або частка `rejected` серед записів за останні 14 днів > 2% (`HEALTH_REJECTED_*`), або є `failed`-понад-cap записи, або папка недоступна |
...
`MIN_FILE_AGE_SEC` (300), `FILE_GLOB` (`*.txt`), `UBKI_CONCURRENCY` (8),
`UBKI_MAX_RPS` (25; UBKI дозволяє ≤30/с), `TELEGRAM_*` (алерти опційні).
